...
class Settings {
  ...
  constructor(nop, context, repo, config, ref, suborg) {
    ...
    /** @type {any} Removed typing to avoid complex typing issues */
  ...
  async updateRepos(repo) {
    // Not needed - all calls are preceeded by loading config
  ...
  async updateAll() {
    ...
    await Promise.all(repositories.map(async repository => {
      ...
      await this.updateRepos({ owner: repository.owner.login, repo: repository.name })
    }))
  }
  ...
  async getSubOrgConfigs() {
    try {
      ...
      for (const override of overridePaths) {
        ...
        if (!data) { continue } // Note-to-self: I think this should be `continue`? If one file is missing, this basically halts reading the rest...
      ...
      // Note: we still load all suborgs above in case the final config is a merged config
  ...
  storeSubOrgConfigIfNoConflicts(subOrgConfigs, overridePath, repoNameOrGlob, data) {
    // Note-to-self: this is probably the place to insert complex merging logic
