  ...
  sync() {
    ...
    if (this.entries) {
      ...
      return this.find().then(existingRecords => {
        ...
        this.log.debug(`Results of comparing ${this.constructor.name} diffable target ${JSON.stringify(existingRecords, null, 2)} with source ${JSON.stringify(filteredEntries, null, 2)} is ${JSON.stringify(results, null, 2)}`)
        // this.log.info(results, `Results of comparing ${this.constructor.name} diffable target`)
